...
const ICONS: any[] = [
  'book-outline', 'moon-outline', 'sparkles-outline',
  'flame-outline', 'leaf-outline', 'star-outline',
  'aperture-outline', 'color-wand-outline',
];

const StoryCard = ({ title, index, onPress }: { title: string; index: number; onPress: () => void }) => {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {!loading && !error && filtered.length > 0 && (
          <View style={styles.countRow}>
            <View style={styles.countPill}>
              <Text style={styles.countText}>{filtered.length} stories</Text>
            </View>
          </View>
        )}
        ...
        {!loading && error && (
          <View style={styles.centeredState}>
            ...
            <TouchableOpacity style={styles.retryBtn} onPress={fetchStories}>
        ...
        {!loading && !error && filtered.map((item, index) => (
          <StoryCard
            ...
            title={item.title}
            index={index}
            onPress={() => navigation?.navigate('StoryDetail', { item })}
          />
        ))}
