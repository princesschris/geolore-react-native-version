...
const ICONS: any[] = [
  'eye-outline', 'moon-outline', 'sparkles-outline',
  'star-outline', 'flame-outline', 'prism-outline',
  'infinite-outline', 'leaf-outline',
];

const BeliefCard = ({ title, index, onPress }: { title: string; index: number; onPress: () => void }) => {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            ...
            <Text style={styles.headerTitle}>Beliefs</Text>
          </View>
          <View style={styles.headerBadge}>
            <Ionicons name="eye" size={28} color="#F5A623" />
          </View>
        </View>
        {!loading && !error && filtered.length > 0 && (
          <View style={styles.countRow}>
            <View style={styles.countPill}>
              <Text style={styles.countText}>{filtered.length} beliefs</Text>
            </View>
          </View>
        )}
        {loading && (
          <View style={styles.centeredState}>
            ...
            <Text style={styles.stateText}>Loading {tribe} beliefs...</Text>
          </View>
        )}
        ...
        {!loading && !error && filtered.length === 0 && (
          <View style={styles.centeredState}>
            <Ionicons name="eye-outline" size={48} color="#C4A882" />
            <Text style={styles.stateText}>No beliefs found for {tribe}</Text>
          </View>
        )}

        {!loading && !error && filtered.map((belief, index) => (
          <BeliefCard
            key={belief.id}
            title={belief.title}
            index={index}
            onPress={() => navigation?.navigate('BeliefDetail', { belief, index })}
          />
        ))}
