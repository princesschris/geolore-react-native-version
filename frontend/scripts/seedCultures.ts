...
async function generateEmbedding(text: string): Promise<number[] | null> {
  ...
  try {
    ...
    console.warn('  Embedding fetch failed:', e.message);
    return null;
  }
}
...
function extractFashionSections(body: string): FashionSections {
  ...
  const parts   = [description, materials, worn_by, occasions, significance, modern_usage].filter(Boolean) as string[];
...
function parseCultureContent(culture: string, category: string, md: string): CultureContent[] {
  ...
  if (subsections.length === 0) {
    rows.push({ culture, category, title: category.charAt(0).toUpperCase() + category.slice(1), content: md, sort_order: 0 });
    return rows;
  }

  subsections.forEach(({ title, body }, index) => {
    rows.push({ culture, category, title: stripMarkdown(title), content: body, sort_order: index });
  });
...
function parseFashionContent(culture: string, md: string): CultureContent[] {
  ...
  for (const { title: h2Title, body: h2Body } of splitByH2(md)) {
    const h3Items = splitByH3(h2Body);
    if (h3Items.length === 0) continue;

    for (const { title: h3Title, body: h3Body } of h3Items) {
      const cleanTitle = stripMarkdown(h3Title);
      if (seen.has(cleanTitle.toLowerCase())) continue;
      ...
      rows.push({
        ...
        title:    cleanTitle, subtitle: stripMarkdown(h2Title),
...
function parseFoodItems(culture: string, md: string): FoodItem[] {
  ...
  splitByH2(md).forEach(({ title, body }, index) => {
    ...
    const ingredients   = extractBullets(body.match(/###\s*Ingredients\s*([\s\S]*?)(?=###|$)/i)?.[1] ?? '');
    const steps         = extractNumberedSteps(body.match(/###\s*(?:Preparation|Steps|How to Cook|Method)\s*([\s\S]*?)(?=###|$)/i)?.[1] ?? '');

    if (ingredients.length === 0 && steps.length === 0) return;
    items.push({
      culture, name: cleanName,
      native_name: nativeMatch   ? nativeMatch[1].trim()   : undefined,
      category:    categoryMatch ? categoryMatch[1].trim() : undefined,
      ingredients, steps, sort_order: index,
    });
  });
  return items;
}
...
function parseProverbs(culture: string, md: string): Proverb[] {
  ...
  const dataRows = md.split('\n')
    ...
    .filter(l => !l.includes('---') && !l.toLowerCase().includes('proverb') && !l.toLowerCase().includes('translation'));

  for (const row of dataRows) {
    const cells = row.split('|').map(c => c.trim()).filter(Boolean);
    if (cells.length >= 2) {
      proverbs.push({ culture, native_text: cells[0], translation: cells[1], meaning: cells[2], explanation: cells[3] });
    }
  }
  return proverbs;
}
...
async function seedCultureContent(rows: CultureContent[]): Promise<void> {
  ...
  for (let i = 0; i < rows.length; i++) {
    const row  = rows[i];
    const text = `${row.title}. ${stripMarkdown(row.content)}`;
...
async function seedFoodItems(rows: FoodItem[]): Promise<void> {
  ...
  const { error } = await supabase.from('food_items').upsert(rows, { onConflict: 'culture,name', ignoreDuplicates: false });
...
async function seedProverbs(rows: Proverb[]): Promise<void> {
  ...
  const { error } = await supabase.from('proverbs').upsert(rows, { onConflict: 'culture,native_text', ignoreDuplicates: false });
...
async function main(): Promise<void> {
  ...
  if (!HF_TOKEN) {
    console.warn('⚠  HF_TOKEN not set. You may hit rate limits on the free tier.');
    console.warn('   Get a free token at huggingface.co → Settings → Access Tokens\n');
  }

  if (!fs.existsSync(CULTURES_DIR)) { console.error(`Directory not found: ${CULTURES_DIR}`); process.exit(1); }
  ...
  for (const file of files) {
    ...
    const md       = fs.readFileSync(path.join(CULTURES_DIR, file), 'utf-8');
    ...
    for (const [sectionTitle, sectionBody] of Object.entries(sections)) {
      ...
      if      (key.includes('food'))      foodRows.push(...parseFoodItems(culture, sectionBody));
      else if (key.includes('proverb'))   proverbRows.push(...parseProverbs(culture, sectionBody));
      else if (key.includes('fashion'))   contentRows.push(...parseFashionContent(culture, sectionBody));
      else if (
        key.includes('history') || key.includes('tradition') || key.includes('culture') ||
        key.includes('festival') || key.includes('belief') || key.includes('stories') || key.includes('language')
      ) {
        const category =
